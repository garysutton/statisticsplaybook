...
p1 <- ggplot(mtcars, aes(x = mpg, y = wt)) + 
  geom_point() +
  labs(title = "Miles per Gallon vs. Weight in Select 1973-74 Automobiles",
       subtitle = "correlation coefficient = -0.87",
       x = "Miles per Gallon", y = "Weight (x 1,000 lbs.)") + 
...
p2 <- ggplot(mtcars, aes(x = am, y = mpg)) +
  ...
  labs(title = "Average Miles per Gallon by Cylinder Numbers and Transmission Type",
       x = "", y = "Miles per Gallon (mpg)", subtitle = "Select 1973-74 Automobiles") + 
  stat_summary(fun = mean, geom = "point", shape = 20, size = 4, color = "white", fill = "white") + 
  theme(plot.title = element_text(face = "bold")) +
  facet_wrap(~cyl) +
...
p3 <- ggplot(tibble1, aes(x = reorder(gear, -mean), y = mean)) + 
  ...
  labs(title = "Average Miles per Gallon by Number of Forward Gears",
       subtitle = "Select 1973-74 Automobiles", 
       x = "Number of Forward Gears", y = "Average Miles per Gallon (mpg)") + 
  geom_text(aes(label = (trunc(mean)), vjust = -0.3)) +
  theme(plot.title = element_text(face = "bold")) +
  ylim(0, 26) 

p4 <- ggplot(mtcars, aes(x = mpg)) + 
  geom_histogram(fill = "indianred1", color = "indianred4", bins = 6) + 
  labs(title = "Frequency Distribution: Miles per Gallon in Select 1973-74 Automobiles", subtitle = "n = 32",
       x = "Miles per Gallon (mpg)", y = "Frequency") +
  scale_y_continuous(breaks = c(2, 4, 6, 8, 10)) +
  theme(plot.title = element_text(face = "bold")) +
  annotate("text", x = 28.6, y = 8.5, label = "Highest MPG: Toyota Corolla", size = 3) + 
  annotate("text", x = 28.6, y = 8, label = "Lowest MPG: Cadillac Fleetwood and Lincoln Continental",
           size = 3)
...
packages %>%
  group_by(date) %>%
  tally() -> tibble2
filter(tibble2, date >= 2015 & date <= 2022) -> tibble2
mutate(tibble2, pct = (n/19305)*100) -> tibble2
print(tibble2)
...
p5 <- ggplot(tibble2, aes(x = date, y = n)) + 
  ...
  labs(title = "Number of Available CRAN Packages by Publication Year",
       subtitle = "2015-22", 
       x = "Year", y = "Count", caption = "March 2023 snapshot") + 
...
p6 <- ggplot(df1, aes(x = platform, y = jobs)) + 
  geom_bar(stat = "identity", width = .5, color = c("dodgerblue1", "grey40", "grey40", "grey40", "grey40", "grey40"),
           fill = c("dodgerblue1", "grey40", "grey40", "grey40", "grey40", "grey40")) +
  labs(title = "U.S. Job Opportunity Counts",
       subtitle = "March 2023", 
       x = "Programming Language", y = "Count") + 
